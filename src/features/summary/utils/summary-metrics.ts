...
import type {
  ...
  SummaryPeriod,
  SummaryPieSlice,
  SummaryRecentListCard,
  SummaryData,
} from '../types';

const DAY_IN_MS = 24 * 60 * 60 * 1000;

const EMPTY_DAILY_SERIES: SummaryDatePoint[] = [];
...
export const buildPieSlices = (lists: List[], periodItems: ListItem[]): SummaryPieSlice[] => {
...
const buildDailySeries = (items: ListItem[]): SummaryDatePoint[] => {
...
const buildItemVariation = (key: string, items: ListItem[]): SummaryItemVariation | null => {
...
export const buildItemVariations = (
  ...
  return [...groupedItems.entries()]
    ...
    .filter((item): item is SummaryItemVariation => item !== null);
};

export const splitItemVariations = (variations: SummaryItemVariation[]) => {
...
export const findItemVariationByKey = (
  variations: SummaryItemVariation[],
  key?: string,
): SummaryItemVariation | null => {
  if (!key) return null;
...
export const calculateAverageDailyVariationPercent = (series: SummaryDatePoint[]): number => {
...
export const getPeriodLabel = (period: SummaryPeriod): string => {
...
export const parseSummaryPeriod = (value?: string): SummaryPeriod => {
